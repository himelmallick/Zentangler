...
cat("Running single-view comparator HPC task\n")
...
for (rep_i in seq_len(nrep)) {
  ...
  if (is.null(mae_i)) {
    errors <- c(errors, paste0("rep_", rep_i, ": missing trainMae"))
    next
  }
  ...
  for (method in methods) {
    ...
    for (view in names(blocks)) {
      ...
      if (inherits(res, "try-error")) {
        errors <- c(errors, paste0(key, ": ", conditionMessage(attr(res, "condition"))))
...
cat("Finished single-view comparator task", task_id, "\n")
cat("Saved:", out_rds, "\n")
print(summary)
